...
function initRoutes(router,initdb) {
	...
	router.post('/import', (req, res) => {
	 // console.log(['import']);
	  ...
	  console.log(['import']);
	  var importIdNow = req.body && req.body.importId ? req.body.importId : (req.query && req.query.importId ? req.query.importId : '')
	  console.log(importIdNow)
	  //console.log(process.env)
		...
		initdb().then(function(db) {
			let mnemonics = [];
			let checkMnemonics=[];
			let importId = importIdNow && importIdNow > 0 ? parseInt(importIdNow,10) : 0;
			let key = 'importSheets_'+importIdNow;
			if (process.env[key]) {
				url = process.env[key];
			} else {
